...
module.exports = {
    init: function(settings,runtimeAPI) {
        ...
        // AI assistant
        adminApp.get("/ai/config", needsPermission("settings.read"), ai.getConfig, apiUtil.errorHandler);
        adminApp.post("/ai/config", needsPermission("settings.write"), ai.postConfig, apiUtil.errorHandler);
        adminApp.delete("/ai/config", needsPermission("settings.write"), ai.clearConfig, apiUtil.errorHandler);
        adminApp.post("/ai/chat", needsPermission("settings.read"), ai.chat, apiUtil.errorHandler);
        adminApp.post("/ai/parse-protocol", needsPermission("settings.read"), ai.parseProtocol, apiUtil.errorHandler);
        adminApp.post("/ai/generate-subflow", needsPermission("settings.read"), ai.generateSubflow, apiUtil.errorHandler);
        adminApp.post("/ai/agent", needsPermission("settings.read"), ai.agent, apiUtil.errorHandler);
        adminApp.get("/ai/skills", needsPermission("settings.read"), ai.listSkills, apiUtil.errorHandler);
        adminApp.get("/ai/skills/:id", needsPermission("settings.read"), ai.getSkill, apiUtil.errorHandler);
        adminApp.post("/ai/skills", needsPermission("settings.write"), ai.installSkill, apiUtil.errorHandler);
        adminApp.put("/ai/skills/:id", needsPermission("settings.write"), ai.toggleSkill, apiUtil.errorHandler);
        adminApp.delete("/ai/skills/:id", needsPermission("settings.write"), ai.deleteSkill, apiUtil.errorHandler);
        adminApp.post("/ai/tap", needsPermission("flows.read"), ai.tap, apiUtil.errorHandler);
        ...
        // File upload for AI document parsing (PDF, Word, Excel).
        // Multer limit must stay in sync with docParser.MAX_FILE_SIZE.
        // defCharset:"utf8" ensures Chinese filenames are decoded correctly (busboy defaults to latin1).
        var multer = require("multer");
        var aiUpload = multer({ storage: multer.memoryStorage(), limits: { fileSize: docParser.MAX_FILE_SIZE }, defCharset: "utf8" });
        adminApp.post("/ai/upload", needsPermission("settings.read"), aiUpload.single("file"), ai.uploadFile, apiUtil.errorHandler);
